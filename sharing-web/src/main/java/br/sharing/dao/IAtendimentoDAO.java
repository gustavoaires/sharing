...
public interface IAtendimentoDAO extends JpaRepository<Atendimento, Long> {
	...
	public List<Atendimento> findByLoginAndStatusRecebidos(@Param("login") String login, 
	...
	@Query("select a from atendimento a where a.idPediuAjuda = :login and a.status = :status")
	public List<Atendimento> findByLoginAndStatusFeitos(@Param("login") String login, 
			@Param("status") String status);
}
